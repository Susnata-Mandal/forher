/* ===========================================================
   ...
   Special Girlfriend's Day Edition
   ===========================================================
...
// =============================================================
// PARTICLE — Floating pollen & hearts
// =============================================================
class Particle {
    constructor(cw, ch) {
        this.cw = cw;
    ...
    reset(initial = false) {
        ...
        this.hue = 330 + Math.random() * 30; // Pink/magenta shades
        this.brightness = 75 + Math.random() * 20;
        this.flickerPhase = Math.random() * Math.PI * 2;
        this.isHeart = Math.random() > 0.4; // 60% chance to spawn a heart
    }
    ...
    draw(ctx, bloomLevel = 1) {
        const t = this.life / this.maxLife;
        const flicker = 0.5 + 0.5 * Math.sin(this.life * 0.08 + this.flickerPhase);
        const alpha = t * 0.85 * flicker * Math.max(0.2, bloomLevel);
        ...
        if (this.isHeart && bloomLevel > 0.1) {
            // Draw floating heart shape
            const size = (this.radius + 3) * (0.8 + bloomLevel * 0.5);
            ...
            // Standard glowing sparkle point
            ctx.beginPath();
...
class FlowerBloomApp {
    constructor() {
        ...
        this.peaceOverlayEl = document.getElementById('peaceOverlay');
        this.peaceTextEl = document.getElementById('peaceText');
        this.peaceProgressFillEl = document.getElementById('peaceProgressFill');

        // Peace-sign ("✌️") surprise gesture
        this.SURPRISE_URL = 'https://templatediaryy.my.canva.site/';
        this.PEACE_HOLD_MS = 1100; // how long the gesture must be held
        this.peaceStartTime = null;
    ...
    onHandResults(results) {
        ...
        if (this.handsDetected > 0) {
            for (let i = 0; i < this.handsDetected; i++) {
                ...
                // Inverted label so mirrored selfie camera matches physical hands
                const isLeft = handedness && handedness.label === 'Right';
        ...
        for (let i = 0; i < this.handsDetected; i++) {
            if (this.isPeaceSign(this.handLandmarks[i])) {
                peaceDetected = true;
                break;
            }
        }
        this.updatePeaceGesture(peaceDetected);
    }
     ...
     * than its PIP joint does — robust to hand rotation/tilt, unlike
     * comparing raw y-coordinates.
     */
    isFingerExtended(lm, tipIdx, pipIdx, wristIdx = 0) {
        return this.dist(lm[tipIdx], lm[wristIdx]) > this.dist(lm[pipIdx], lm[wristIdx]) * 1.15;
    }

    /** ✌️ Peace sign: index + middle extended, ring + pinky curled */
    isPeaceSign(lm) {
        const indexUp = this.isFingerExtended(lm, 8, 6);
        const middleUp = this.isFingerExtended(lm, 12, 10);
        const ringDown = !this.isFingerExtended(lm, 16, 14);
        const pinkyDown = !this.isFingerExtended(lm, 20, 18);
        return indexUp && middleUp && ringDown && pinkyDown;
    }

    /**
     * Tracks how long the peace sign has been held and drives the
     * surprise overlay. Once held for PEACE_HOLD_MS, navigates to
     * the surprise page.
     */
    updatePeaceGesture(detected) {
        if (this.peaceTriggered) return;

        if (detected) {
            if (this.peaceStartTime === null) this.peaceStartTime = performance.now();
            const held = performance.now() - this.peaceStartTime;
            const progress = Math.min(1, held / this.PEACE_HOLD_MS);
            this.setPeaceProgress(progress);
            ...
            this.peaceStartTime = null;
            this.setPeaceProgress(0);
        }
    }

    setPeaceProgress(progress) {
        if (!this.peaceOverlayEl) return;
        this.peaceOverlayEl.classList.toggle('visible', progress > 0);
        if (this.peaceProgressFillEl) this.peaceProgressFillEl.style.width = `${progress * 100}%`;
    }

    /** Plays a short transition, then opens the surprise page */
    triggerSurprise() {
        if (this.peaceTextEl) this.peaceTextEl.textContent = 'Opening your surprise...';
        this.peaceOverlayEl?.classList.add('visible', 'triggered');
        setTimeout(() => {
            window.location.href = this.SURPRISE_URL;
        }, 900);
    }
    ...
    drawGreetingMessage() {
        ...
        ctx.save();
        // Counter-flip X coordinates so text renders normally over mirrored video
        ctx.translate(cw, 0);
        ctx.scale(-1, 1);
        ...
        const combinedFactor = (this.growth * 0.5 + this.bloom * 0.5);
        const fontSize = Math.floor(22 + combinedFactor * 18);
        const alpha = Math.min(1, this.growth * 1.5);
        ...
        const glowAmount = 10 + this.bloom * 30;
        ctx.shadowBlur = glowAmount;
        ctx.shadowColor = `rgba(255, 100, 160, ${0.6 + this.bloom * 0.4})`;

        const textGrad = ctx.createLinearGradient(centerX - 150, 0, centerX + 150, 0);
        textGrad.addColorStop(0, '#ff9a9e');
        textGrad.addColorStop(0.5, '#fecfef');
        textGrad.addColorStop(1, '#ff758c');

        ctx.fillStyle = textGrad;
        ctx.fillText("Happy Girlfriend's Day Sweetheart", centerX, centerY);

        if (this.bloom > 0.3) {
            ctx.font = `300 ${Math.floor(fontSize * 0.45)}px 'Inter', sans-serif`;
            ctx.fillStyle = `rgba(255, 230, 240, ${this.bloom})`;
            ctx.shadowBlur = 10;
            ctx.fillText("You make my heart bloom ♥", centerX, centerY + fontSize * 0.8);
        }
    ...
    animate(timestamp) {
        ...
        for (const p of this.particles) {
            ...
            p.draw(ctx, this.bloom);
        }

        // ---- Main Flowers (Both Sides) ----
        if (this.growth > 0.005) {
            const flowerPositions = [
                { baseX: cw * 0.25, dir: 1 },  // Left side flower
                { baseX: cw * 0.75, dir: -1 }  // Right side flower
            ];

            const stemH = ch * 0.42 * this.growth;
            const flowerScale = 1.15 * this.growth;

            for (const pos of flowerPositions) {
                const stemData = this.drawStem(pos.baseX, ch * 0.95, stemH, totalWind * pos.dir);
                const tip = stemData.tip;
                const pts = stemData.pts;

                // Branches for each flower
                const branchConfigs = [
                    { heightRatio: 0.35, direction: -1 * pos.dir, lengthFactor: 0.14, scaleFactor: 0.4 },
                    { heightRatio: 0.55, direction: 1 * pos.dir, lengthFactor: 0.12, scaleFactor: 0.42 },
                    { heightRatio: 0.72, direction: -1 * pos.dir, lengthFactor: 0.1, scaleFactor: 0.38 }
                ];
